...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_gitcommit", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_gitcommit": ["*.pyi", "py.typed"],
        "tree_sitter_gitcommit.queries": ["*.scm"],
    },
    ext_package="tree_sitter_gitcommit",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_gitcommit/binding.c",
                "src/parser.c",
            ],
            define_macros=[
